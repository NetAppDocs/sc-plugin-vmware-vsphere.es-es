...
---
= Actualiza desde una versión anterior del SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: La actualización a SCV 6.2 solo es compatible con VMware vCenter Server 7 update 1 y versiones posteriores, para VMware vCenter Server anteriores a la versión 7 update 1, debes seguir usando SCV 4.7. La actualización es disruptiva en las versiones no compatibles de VMware vCenter Server.
...
CAUTION: Antes de realizar la actualización, comprueba que tu entorno cumpla los requisitos del sistema. Consulta link:scpivs44_deployment_planning_and_requirements.html["SnapCenter Plug-in para VMware vSphere planificación de la implementación y requisitos"] para obtener más información.
...
Para obtener la información más reciente sobre las versiones compatibles, consulta link:https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["Herramienta de matriz de interoperabilidad de NetApp (IMT)"^].
...
+
.. Inicia sesión en link:https://mysupport.netapp.com/products/index.html["Sitio de soporte de NetApp"^].
