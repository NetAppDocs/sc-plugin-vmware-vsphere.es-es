...
---
= Crea políticas de backup para VMs y datastores en el SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
Para obtener información sobre cómo los administradores asignan recursos a los usuarios, consulte https://docs.netapp.com/us-en/snapcenter/get-started/rbac-snapcenter.html["Información de SnapCenter sobre el uso del control de acceso basado en roles"^] .
...
+
** Configure clusters y mediador como se describe en el informe técnico: https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/mediator-install-task.html["Configurar el mediador de ONTAP y los clústeres para la sincronización activa de snapmirror"].
...
+
No utilice los siguientes caracteres especiales en máquinas virtuales, almacenes de datos, clústeres, políticas, backup o nombres de grupos de recursos: % & * $ # @ ! \ / : * ? " < > - | ; ', y espacio.
...
NOTE: El valor de retención máxima es 1018 backups. Se producirá un error en los backups si la retención se establece en un valor superior a la versión de ONTAP subyacente. Esto también se aplica a almacenes de datos de expansión.
