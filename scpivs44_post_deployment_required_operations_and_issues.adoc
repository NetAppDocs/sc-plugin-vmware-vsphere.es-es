...
---
= Operaciones necesarias y problemas posteriores al despliegue para el SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Borre la memoria caché del navegador y luego verifique si la interfaz de usuario funciona correctamente.
+
...
.Pasos
. Inicie sesión en la interfaz de usuario de administración del SnapCenter Plug-in for VMware vSphere usando el formato `\https://<appliance-IP-address>:8080` .  Utilice el nombre de usuario de administrador, la contraseña y los detalles del token MFA para iniciar sesión.  El token MFA se puede generar desde la consola de mantenimiento.
...
* Puede registrar el SnapCenter Plug-in for VMware vSphere con SnapCenter Server mediante la interfaz de usuario de SnapCenter para agregar un host de tipo “vSphere”.
+
...
.Pasos
. En el panel de navegación izquierdo de la interfaz de usuario de SnapCenter , seleccione *Hosts*.
