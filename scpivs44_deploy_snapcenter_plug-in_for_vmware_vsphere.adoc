...
* Para proteger las máquinas virtuales en los almacenes de datos vVol, primero implemente las ONTAP tools for VMware vSphere .  Para conocer las versiones compatibles de las herramientas ONTAP , consulte la https://imt.netapp.com/matrix/imt.jsp?components=134348;&solution=1517&isHWU&src=IMT["Herramienta de matriz de interoperabilidad de NetApp"^] .  Las herramientas ONTAP aprovisionan y configuran almacenamiento en ONTAP y el cliente web VMware.
