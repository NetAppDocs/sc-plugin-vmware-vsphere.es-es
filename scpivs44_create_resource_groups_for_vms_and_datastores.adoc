...
---
= Crea grupos de recursos para máquinas virtuales y almacenes de datos en el SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Para volúmenes FlexGroup en entornos MetroCluster, si estás utilizando ONTAP 9.8 o 9.9, reinicia el servicio SnapCenter Plug-in for VMware vSphere y resincroniza las relaciones SnapMirror después de una conmutación de sitios o conmutación inversa antes de hacer una copia de seguridad de los grupos de recursos. En ONTAP 9.8, las copias de seguridad pueden quedarse colgadas después de una conmutación inversa; esto se resuelve en ONTAP 9.9.
...
| VCenter Server | Selecciona un servidor vCenter. 
